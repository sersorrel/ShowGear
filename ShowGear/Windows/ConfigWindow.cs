...
public class ConfigWindow : Window, IDisposable
{
    ...
    {
        ImGui.TextWrapped(
            "The Item Dyeing window doesn’t remember the previous state of the Show Gear button, which makes it "
            + "somewhat more difficult for this plugin to deal with. When this setting is enabled, Show Gear will be "
            + "enabled every time you open Item Dyeing, but if you disable Show Gear and want to re-enable it without "
            + "closing and reopening the window, you’ll have to click “Manually adjust visor” to update the model.");
        ImGui.TextWrapped(
            "If you’d rather this plugin not interfere with Item Dyeing at all, disable this option.");
        // can't ref a property, so use a local copy
        var manageItemDyeing = this.configuration.ManageItemDyeing;
        if (ImGui.Checkbox("Manage Item Dyeing", ref manageItemDyeing))
        {
